...
# --- 平台检测 ---
OS_TYPE="$(uname -s)"
PLIST_LABEL="com.openclaw.cache-keepalive-proxy"
PLIST_DIR="${HOME}/Library/LaunchAgents"
PLIST_FILE="${PLIST_DIR}/${PLIST_LABEL}.plist"
LOG_FILE="${INSTALL_DIR}/proxy.log"
ERR_LOG_FILE="${INSTALL_DIR}/proxy.err.log"
...
if [[ "${1:-}" == "--uninstall" || "${1:-}" == "uninstall" ]]; then
  ...
  # 停止并卸载 launchd 服务 (macOS)
  if [[ "$OS_TYPE" == "Darwin" ]] && [ -f "${PLIST_FILE}" ]; then
    launchctl bootout "gui/$(id -u)/${PLIST_LABEL}" 2>/dev/null || true
    launchctl unload "${PLIST_FILE}" 2>/dev/null || true
    rm -f "${PLIST_FILE}"
    echo -e "  ${GREEN}✓${NC} launchd 服务已卸载"
  fi

  # 停止并禁用 systemd 服务 (Linux)
...
if [[ "$OS_TYPE" == "Darwin" ]]; then
  ...
  echo -e "  ${YELLOW}!${NC} 未检测到服务管理器，将跳过服务注册"
fi
...
EOF
  echo -e "  ${GREEN}✓${NC} launchd 服务已注册（随用户登录自动启动）"

  # 启动
  # Note: launchctl load/unload are soft-deprecated since macOS 10.10 in favor of
  # bootstrap/bootout, but they remain fully functional for user LaunchAgents on all
  # current macOS versions (including macOS 15 Sequoia). We use load/unload here for
  # broad compatibility and simpler error semantics.
  if [ -n "$FINAL_UPSTREAM" ]; then
    launchctl unload "${PLIST_FILE}" 2>/dev/null || true
    if ! launchctl load "${PLIST_FILE}"; then
      echo -e "  ${RED}✗${NC} launchctl load 失败，plist 可能有问题："
      echo "    cat ${PLIST_FILE}"
      echo "    tail -20 ${ERR_LOG_FILE}"
    else
      sleep 1
      # Check PID field from launchctl list (column 1: PID or "-" if not running)
      JOB_PID=$(launchctl list 2>/dev/null | awk -v label="${PLIST_LABEL}" '$3==label{print $1}')
      if [[ -n "$JOB_PID" && "$JOB_PID" != "-" ]]; then
        echo -e "  ${GREEN}✓${NC} 代理已启动 (PID ${JOB_PID})"
        ...
        echo "    tail -50 ${ERR_LOG_FILE}"
      fi
    fi
  else
    echo -e "  ${YELLOW}!${NC} 上游地址未设置，服务未启动"
    echo "  编辑 config.conf 后手动启动: launchctl load ${PLIST_FILE}"
  fi
...
EOF
  ...
  # --- 无服务管理器 ---
  echo -e "  ${YELLOW}!${NC} 跳过服务配置"
  echo "  手动启动: CONF_FILE=${INSTALL_DIR}/config.conf node ${INSTALL_DIR}/proxy.js"
fi
...
if [ "${SERVICE_TYPE}" = "launchd" ]; then
  ...
  echo -e "  停止服务: ${CYAN}launchctl unload ${PLIST_FILE}${NC}"
  echo -e "  启动服务: ${CYAN}launchctl load ${PLIST_FILE}${NC}"
elif [ "${SERVICE_TYPE}" = "systemd" ]; then
  echo -e "  查看日志: ${CYAN}journalctl --user -u ${SERVICE_NAME} -f${NC}"
  echo -e "  停止服务: ${CYAN}systemctl --user stop ${SERVICE_NAME}${NC}"
  echo -e "  启动服务: ${CYAN}systemctl --user start ${SERVICE_NAME}${NC}"
else
  echo -e "  手动启动: ${CYAN}CONF_FILE=${INSTALL_DIR}/config.conf node ${INSTALL_DIR}/proxy.js${NC}"
fi
echo ""
echo -e "  ${BOLD}其他：${NC}"
