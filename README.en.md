...
`install.sh` auto-detects macOS and will:

1. copy files into `~/.openclaw/cache-keepalive-proxy/`
2. create the config file
3. register and start a `launchd` service (auto-starts on user login)
...
You can also run manually without `install.sh`:
...
- **Node.js 18+**
- Recommended for macOS long-running use:
  - **macOS 10.15+** (Catalina or later)
  - **launchd** (built-in, auto-configured by `install.sh`)
...
- `sessionSource`
  ...
  - `header.x-claude-code-session-id` (fallback)
  - `system.chat_id`
- `keepaliveEnabled`
...
- Claude Code v2.1.85+ `adaptive` thinking type handled correctly
- Keepalive requests carry `cache-keepalive-proxy/` User-Agent prefix for easy identification in API logs
- Falls back to `x-claude-code-session-id` header when `metadata.user_id` parsing fails
...
- The optional `/cache` slash command requires Python 3 on macOS (install via `brew install python3` or Xcode Command Line Tools)
