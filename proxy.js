#!/usr/bin/env node
/**
 * OpenClaw Cache Keepalive Proxy v2.0.0
 *
...
const LOG = (tag, sid, msg) => console.log(`[${localTimestamp()}] [${tag}] [${sid || '-'}] ${msg}`);
...
try { fs.watch(CONF_FILE, { persistent: false }, () => { if (_reloadTimer) clearTimeout(_reloadTimer); _reloadTimer = setTimeout(reloadConf, 500); }); } catch {}
...
function isClaudeCodeRequest(headers, body) {
  ...
  return userAgent.includes('claude-cli/') ||
    ...
    Boolean(headers?.['x-claude-code-session-id']) ||
...
// extended thinking enabled, effort config, and at least one tool declared.
// This distinguishes them from subagent requests (which lack thinking).
...
function extractSessionInfo(headers, body) {
  try {
    ...
    if (isClaudeCodeRequest(headers, body)) {
      ...
      const sessionId = metadata?.session_id || headers?.['x-claude-code-session-id'];
      if (sessionId) {
        const source = metadata?.session_id
...
function replaceSessionTemplate(sid, body, raw, headers, path, sessionInfo, enableKeepalive, disabledReason = null, requestSentAt = null) {
  ...
  // Adjust first keepalive delay to account for response streaming time.
  // Anthropic cache TTL starts when the request is processed (≈ requestSentAt),
  // not when the response stream completes. For long-running Opus responses,
  // streaming can take 60-120s, which would push a fixed 4m30s delay past the
  // 5-minute cache TTL. Subtract the elapsed streaming time so the keepalive
  // fires relative to cache creation, not response completion.
  let firstDelay = KEEPALIVE_MS;
  if (requestSentAt && enableKeepalive) {
    const streamingElapsed = Date.now() - requestSentAt;
    firstDelay = Math.max(KEEPALIVE_MS - streamingElapsed, 30000); // minimum 30s
    if (streamingElapsed > 10000) {
      LOG('keepalive', sid, `adjusted first keepalive delay: ${(firstDelay / 1000).toFixed(0)}s (streaming took ${(streamingElapsed / 1000).toFixed(1)}s)`);
    }
  }
  scheduleSessionKeepalive(sid, session, firstDelay);
}
...
function sendKeepalive(sid, isRetry = false) {
  ...
  kaBody.stream = false;
  if (kaBody.thinking) {
    if (kaBody.thinking.type === 'enabled') {
      kaBody.thinking.budget_tokens = 128;
    }
    // 'adaptive' type does not accept budget_tokens — leave as-is
  }
  ...
  // Prefix User-Agent for all keepalive requests so they are distinguishable in API logs
    ...
    res.on('end', () => {
      ...
      try {
        ...
        if (res.statusCode >= 400) {
          LOG('keepalive', sid, `error_response: ${d.slice(0, 500)}`);
        }
...
const server = http.createServer((cReq, cRes) => {
  ...
  const chunks = [];
  cReq.on('error', e => {
    LOG('proxy', '-', `client request error: ${e.message}`);
  ...
  cReq.on('end', () => {
    ...
    const requestSentAt = Date.now(); // Track when request was sent (≈ cache creation time)
    const pReq = UP_PROTO.request({
      ...
      if (sid && pRes.statusCode >= 200 && pRes.statusCode < 300) {
        ...
        pRes.on('close', () => {
          ...
          replaceSessionTemplate(sid, body, raw, cReq.headers, cReq.url, sessionInfo, true, null, requestSentAt);
